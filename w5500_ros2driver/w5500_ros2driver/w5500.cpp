...
/* ===========================================================
   Struct ForceData
   -----------------------------------------------------------
   Cache-aligned POD for lock-free access.
   =========================================================== */
...
/* ===========================================================
   Class OpenCoRoCo
   -----------------------------------------------------------
   Real-time optimized network receiver with lock-free design.
   =========================================================== */
class OpenCoRoCo {
public:
    OpenCoRoCo(bool mode = MODE_UDP) 
    ...
    void set_rt_config(int cpu_core, int recv_sleep_us) {
        cpu_core_ = cpu_core;
        sleep_us_ = recv_sleep_us;
    }
    ...
    void start_server() {
        ...
        if (fcntl(sockfd_, F_SETFL, flags | O_NONBLOCK) < 0)
            RCLCPP_WARN(logger_, "Failed to set non-blocking mode");
        ...
        if (!mode_) {
            ...
            flags = fcntl(connfd_, F_GETFL, 0);
            fcntl(connfd_, F_SETFL, flags | O_NONBLOCK);
    ...
    inline uint8_t get_info() const noexcept {
        return data_.info;
    }
...
private:
    ...
    void set_realtime_priority() {
        ...
        if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &param) != 0) {
            RCLCPP_WARN(logger_, 
                "Failed to set RT priority (need CAP_SYS_NICE capability). "
        ...
        if (cpu_core_ >= 0) {
            ...
            if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset) != 0) {
                ...
                RCLCPP_INFO(logger_, "Thread pinned to CPU core %d", cpu_core_);
            }
        }
    }
...
/* ===========================================================
   ...
   -----------------------------------------------------------
   ROS2 node publishing at 1 kHz with minimal overhead.
   =========================================================== */
...
public:
    ForcestickPublisher() : Node("forcestick_pub"), opencoroco_(MODE_UDP), last_seq_(0) {
        ...
        RCLCPP_INFO(this->get_logger(), "ForcestickPublisher initialized at 1 kHz");
        RCLCPP_INFO(this->get_logger(), "  port=%d, cpu_core=%d, sleep=%dus, skip_unchanged=%s",
                    server_port, cpu_core, recv_sleep_us, skip_unchanged ? "true" : "false");
    }
...
private:
    ...
    void timer_callback() {
        ...
        if (skip_unchanged_ && current_seq == last_seq_) {
            return;
        }
        ...
        msg.stamp = builtin_interfaces::msg::Time(this->now()); 
